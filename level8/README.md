...
The executable launches without arguments, but waits once launched.
...
- 4 keywords: "auth ", "reset", "service", "login".
- auth, performs a malloc.
- reset, free auth.
- service, create a strdup of the entry.
- login, call system if the value of ``auth + 32 is not null``.

So, there are several solutions to solve this problem.
</br>
First, you have to do "auth " to initialize the first malloc.
</br>
After that, there are two solutions:
- Either we call services several times to perform several strdup operations.
- Otherwise we'll make a big strdup.
