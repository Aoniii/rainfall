...
The executable requires an argument.
...
We have two functions, including ``n``, which calls ``system``.
</br>
``n`` does not call the main function.
...
You must successfully complete call ``n``, and it will display the ``pass``.
...
We can see that there are two allocations: the first for the ``buffer`` and the second to store the value of ``m``.
</br>
We can therefore overflow the buffer to write to the address of ``m`` and be able to call ``n``.
</br>
With padding, we need to write after 72 characters, 64 buffers + 8 chunk headers.
...
And there you go, we put ``72 characters`` and then the address of ``n in little-endian`` format.
